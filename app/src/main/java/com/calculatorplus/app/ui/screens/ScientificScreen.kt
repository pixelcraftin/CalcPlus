...
fun ScientificScreen(
    ...
    Column(
        ...
        KeypadGrid(
            ...
            // Row 7: nCr, 0, ., ⌫, =
            Row(modifier = Modifier.fillMaxWidth().weight(1f)) {
                KeypadButton(text = if (isSecondActive) "nPr" else "nCr", onClick = { viewModel.onKeyClick(if (isSecondActive) "nPr" else "nCr") }, shapeName = buttonShape, modifier = Modifier.weight(1f), aspectRatio = null)
